...
public class RegisterRequestValidator implements Validator {
	...
	// 이메일 검증을 위한 정규식
	private static final String emailExp = "\"^[a-zA-Z0-9_+&*-]+(?:\\\\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\\\\.)+[a-z\" +\r\n"
			+ "\"A-Z]{2,7}$\";";
	...
	public void validate(Object target, Errors errors) {
		...
		if (regReq.getEmail() == null || regReq.getEmail().trim().isEmpty()) {
			...
			// 에러출력 태그 email이라는 속성에 required 값(에러 코드)을 전달
			// => 대체되는 메시지로 변환
		} else {
...
//		}
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "name", "required"); // 위의 내용과 동일
		// name이라는 필드가 비어있거나 공백문자가 있다면 에러의 name속성에 required값 전달
